<?php
	...
	class PraprosesBaru
	{
		...
		public function tf_idf($term) {
            if ($term != '') {
                $resCount = mysql_query("SELECT * FROM tbindex_baru WHERE term = '$term'");
                $num_rows = mysql_num_rows($resCount);
                if ($num_rows > 0) {
                    $rowcount = mysql_fetch_array($resCount);
                    $count = $rowcount['jumlah'];
                    $count++;
                    mysql_query("UPDATE tbindex_baru SET jumlah = $count WHERE term = '$term' AND id_doc = 29");
                }
                else {
                    mysql_query("INSERT INTO tbindex_baru (term, id_doc, jumlah) VALUES ('$term', 29, 1)");
                }
            }
        }
